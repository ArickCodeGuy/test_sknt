...
export const useCart = defineStore('cart', {
    ...
    actions: {
        changeTab({item_index, variant_index}: {
            variant_index: number,
            item_index: number
        }) : void {
            this.data[item_index].active_variant = variant_index
        },
        changeOption({item_index, variant_index, option_index} : {
            item_index: number,
            variant_index: number,
            option_index: number
        }) : void {
            const option = this.data[item_index].variants[variant_index].options[option_index]
            option.active = !option.active
        },
        changeSelect({item_index, variant_index, select_index, option_index} : {
            item_index: number,
            variant_index: number,
            select_index: number,
            option_index: number
        }) : void {
            this.data[item_index].variants[variant_index].select[select_index].active_option = option_index
        }
    },
    getters: {
        summary(state) {
